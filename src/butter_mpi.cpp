// // butter_mpi.cpp: MPI wrapper for pre-partitioned subgraphs
...
//     try {
//         P = parse(argc, argv);
//         // --- Validation for MPI Version ---
//         if (P.subgraph_dir.empty()) {
//             throw std::runtime_error("Missing required argument: --subgraph_dir <directory>");
//         }
//         if (!P.file.empty()) {
//              if(mpi_rank == 0) std::cerr << "[MPI Rank 0] Warning: --file provided but ignored in MPI mode (using --subgraph_dir)." << std::endl;
//         }
//         // Check if subgraph directory exists
//         if (!std::filesystem::exists(P.subgraph_dir) || !std::filesystem::is_directory(P.subgraph_dir)) {
//              throw std::runtime_error("Subgraph directory not found or not a directory: " + P.subgraph_dir);
//         }
//         // --- End Validation ---

//     } catch (const std::exception &e) {
//         if (mpi_rank == 0) {
//             std::cerr << "[MPI Rank 0] Error parsing arguments: " << e.what() << "\n";
//             std::cerr << "[MPI Rank 0] Usage: mpirun -np N ./butter_mpi --subgraph_dir <dir> --L <l> [--rank ...] [--agg ...] [--threads T]\n";
//         }
//         MPI_Abort(MPI_COMM_WORLD, 1); return 1;
//     }

//     Graph G; // Each rank has its own Graph structure for its subgraph
//     std::vector<vid> rank_of;
//     double t_load_rank_start = 0, t_load_rank_end = 0;
//     std::string subgraph_filename;

//     try {
//         // --- Phase 1: Load Assigned Subgraph & Compute Local Rank ---
//         t_load_rank_start = MPI_Wtime();

//         // Construct subgraph filename based on rank
//         subgraph_filename = P.subgraph_dir + "/subgraph_" + std::to_string(mpi_rank) + ".txt";

//         // Load the assigned subgraph
//         // load_graph throws if file doesn't exist/can't be opened
//         if (mpi_rank == 0) {
//              std::cout << "[MPI Rank 0] Info: Assuming global node IDs in subgraph files." << std::endl;
//              std::cout << "[MPI Rank 0] Info: Ranking (degree/degcore) will be based on local subgraph data." << std::endl;
//         }
//         G = load_graph(subgraph_filename, P.L);

//         // Compute ranking based *only* on the loaded subgraph G
//         auto order = make_rank(G, P.rank, P.seed);

//         // Compute rank_of array for the local subgraph context
//         if (G.L > 0) {
//              // rank_of needs to be sized based on G.L, which reflects L_param and max L ID in *this* subgraph
//              if (order.size() != G.L) throw std::runtime_error("Rank size mismatch for subgraph " + std::to_string(mpi_rank));
//              rank_of.resize(G.L);
//              for (vid r = 0; r < G.L; ++r) {
//                  vid node_id = order[r];
//                  if (node_id >= G.L) throw std::runtime_error("Rank node ID OOB for subgraph " + std::to_string(mpi_rank));
//                  rank_of[node_id] = r;
//              }
//         } else if (!order.empty()) {
//             throw std::runtime_error("Subgraph " + std::to_string(mpi_rank) + " L=0 but rank non-empty.");
//         }
//         t_load_rank_end = MPI_Wtime();

//         // --- Phase 2: Local Butterfly Counting ---
//         uint64_t local_ans = 0;
//         double t_start_count = MPI_Wtime();
//         int nT_to_use = P.threads;

//         // Call the SIMPLIFIED count_par on the locally loaded graph G
//         switch (P.agg) {
...
//                       << " nodes=" << world_size
//                       << " thr_per_node=" << nT_to_use
//                       << " rank=" << static_cast<int>(P.rank)
//                       << " agg=" << P.agg
//                       << "\n";
//              std::cout << "[mpi] Info: Processed subgraphs from directory: " << P.subgraph_dir << std::endl;
//         }
