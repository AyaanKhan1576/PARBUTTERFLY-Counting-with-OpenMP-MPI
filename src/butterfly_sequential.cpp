...
/****************************************************************************************
 * butterfly_sequential.cpp  –  feature-complete, deterministic butterfly counter
 *
 ...
 *
 * Notes
 *   • Incremental ΔC2 counting in HASH + HISTO removes flush loss.
 *   • SORT and BATCH aggregate by sorting their buffer at flush time.
 *   • FLUSH_BYTES (8 MiB) caps memory but does not affect correctness.
 *****************************************************************************************/
#include <bits/stdc++.h>
...
struct AggSort{
    ...
    void flush(){
        ...
        for(size_t i=0;i<buf.size();){
            ...
            size_t f=j-i; total += (uint64_t)f*(f-1)/2; i=j;
        }
        buf.clear();
    }
...
/* BATCH – fixed chunk size then sort */
...
/* HISTO – incremental choose-two with dense vector */
struct AggHisto{
    std::vector<int> hist; std::vector<vid> touched; uint64_t total=0;
...
/* ───────────────────── Generic driver ─────────────────────── */
template<class Agg>
uint64_t count_bf(const Graph& G,
...
int main(int argc,char**argv)
{
    try{
        ...
        uint64_t ans=0;
        switch(P.agg){
            case Params::HASH : ans=count_bf<AggHash >(G,rank_of); break;
            case Params::SORT : ans=count_bf<AggSort >(G,rank_of); break;
            case Params::BATCH: ans=count_bf<AggBatch>(G,rank_of); break;
            case Params::HISTO: ans=count_bf<AggHisto>(G,rank_of); break;
        }
        double t2=clock()/double(CLOCKS_PER_SEC);
